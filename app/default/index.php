...
$fp = fopen($configs['invidious'].$configs['default'], 'r');
...
echo '<channel><title>Invidious</title>';
echo '<atom:link href="'.$configs['hostname'].'" rel="self" type="application/rss+xml" />';
echo '<description>Trending</description>';
echo '<link>'. xml_entities($configs['hostname']) .'</link>';
echo '<image><url>'.$configs['hostname'].'images/icon.png</url><title>Invidious</title><link>'.$configs['hostname'].'</link></image>';
echo '<item><title>Search Videos</title>';
echo '<link>'.xml_entities($configs['hostname'].'search/').'</link>';
echo '<enclosure url="'.$configs['hostname'].'search/" type="application/rss+xml"/></item>';
for ($x = 0; $x < $json_count; $x++) {
	echo '<item><title>'.xml_entities($json[$x]-> title).'</title>';
	echo '<link>'.xml_entities($configs['riitube'].$json[$x]-> videoId).'</link>';
	echo '<guid>'.xml_entities($configs['riitube'].$json[$x]-> videoId).'</guid>';
	$published = new DateTime('@'.$json[0]-> published);
	echo '<pubDate>'.date_format($published,"D, d M Y H:i:s O").'</pubDate>';
	echo '<enclosure url="'.xml_entities($configs['riitube'].$json[$x]-> videoId).'" type="video/mp4" />';
